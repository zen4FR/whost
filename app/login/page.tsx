...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    console.log('Checking Firebase auth:', auth);
    if (auth) {
      setFirebaseStatus('✅ Firebase ready');
    } else {
  ...
  useEffect(() => {
    console.log('Auth state:', { loading, user: user?.email, userRole });
    ...
    return (
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          <p>Loading...</p>
          <p style={{ fontSize: '12px', color: '#666', marginTop: '10px' }}>{firebaseStatus}</p>
        </div>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <div style={styles.infoBox}>
          <p style={styles.infoText}>By continuing, you agree to our Terms of Service and Privacy Policy</p>
        </div>

        <div style={styles.supportBox}>
          <span>📞</span>
          <span>Need help? WhatsApp us at +977-9860336777</span>
        </div>
      </div>
    </div>
  );
